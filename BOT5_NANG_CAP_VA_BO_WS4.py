...
Bản nâng cấp “siêu chính xác”:
- Regime filter (ADX & ATR%) → tăng ngưỡng confirm khi sideway/biến động mạnh.
- Wick-to-body filter → nến xấu thì tăng ngưỡng confirm.
- Session filter → ngoài giờ thanh khoản cao, tăng ngưỡng confirm.
- 2-step confirm (15 phút) → xác nhận sau khi điều kiện bền vững thêm một nhịp.
...
# Ngưỡng/bumps có thể chỉnh bằng env:
...
# Giờ vàng theo UTC (ví dụ 12–20 UTC: London→NY overlap); định dạng "start-end"
...
def score_frame(ind, pat, funding=None, oi=None, tf="1h"):
    ...
    # Funding / OI (để nguyên logic cũ; có thể nâng cấp z-score sau)
    if tf=="1h":
    ...
    # Regime info for reasons (bản thân score không cưỡng ép; bump confirm ở tầng policy)
...
class Bot5Ultra:
    def __init__(self):
        ...
        self.states={c: {"last_sig_1h":None,"last_bias_1h":0,"bars":COOLDOWN_BARS_1H,
                         "pending_dir": None, "pending_ready_at": 0.0} for c in self.coins}
    ...
    def _session_confirm_bump(self) -> float:
        ...
        try:
            ...
            for win in SESSION_UTC_WINDOWS:
                ...
                a,b = win.split("-")
                a=int(a); b=int(b)
                if a<=h<b:  # bên trong 1 trong các khung
                    return 0.0
    ...
    def one_coin_round(self, coin: str) -> Optional[float]:
        ...
        try:
            ...
            if not df1h.empty:
                ...
                # body nên >= 50% range; upper/lower không quá dài
            ...
            if want_confirm and st["pending_dir"] is None and not note.startswith("🧯"):
                ...
                st["last_sig_1h"]=s1h
                st["bars"]=0
                # Không gửi ngay; chờ lần sau
                send_ok = False
                pending_reason = "⏳ pending confirm (2-step)"
            # Nếu đang pending đúng hướng và đã tới giờ, vẫn đủ điều kiện → gửi
            elif want_confirm and st["pending_dir"] == want_dir and now_ts >= st.get("pending_ready_at", 0):
                send_ok = True
                pending_reason = ""
                # reset pending sau khi gửi
                st["pending_dir"] = None
                st["pending_ready_at"] = 0.0
            else:
                send_ok = False
                pending_reason = ""
                # Nếu mất điều kiện confirm hoặc đổi hướng → huỷ pending
            ...
            if send_ok and self.confirmed_sent<MAX_CONFIRMED_PER_ROUND and not self.tg.paused and price and not note.startswith("🧯"):
                ...
                # Không gửi; log kèm lý do pending nếu có
                st["bars"]+=1
