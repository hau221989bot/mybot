...
# -*- coding: utf-8 -*-
"""
BOT5 Ultra (REST-only, Integrated Upgrades) — v2
- REST-only (không dùng WebSocket)
- Nâng cấp chính so với bản bạn đang chạy:
  1) **/gc thật sự ép Auto-Tune chạy ngay** (bỏ qua TUNE_INTERVAL_MIN).
  2) Thêm **ALERT_ONLY** (chỉ cảnh báo, không mở vị thế) và **PAPER_TRADE** (mặc định True; bot hiện chỉ paper, chưa đặt lệnh thật).
  3) **Trailing SL & TP1**: cập nhật **persist** vào DB (không mất SL mới giữa các vòng).
  4) Auto-Tune: tune trên **union** {coins đang quét} ∪ {coins đang mở vị thế}.
  5) Bổ sung lệnh: **/help**, **/close COIN**, **/flatten** (đóng tất cả), hiển thị trạng thái rõ hơn trong **/status**.
  6) /set hỗ trợ bool: on/off/true/false/1/0.
  7) Một số kiểm soát anti-spam, log, housekeeping giữ nguyên/điều chỉnh nhẹ.

Ghi chú:
- Đây là bản "full chức năng" theo logic hiện tại (paper-trade). Nếu muốn đặt lệnh thật, cần bổ sung ký API và private REST — không nằm trong phạm vi an toàn ở bản này.
"""
...
def safe_json(url: str, params=None):
    try:
        ...
        if used and used.isdigit() and int(used) >= 1100:  # soft guard near 1200
            time.sleep(0.5)
        return r.json()
...
class DB:
    def __init__(self, path=DB_FILE):
        ...
        c = self.conn.cursor()
        c.execute("""CREATE TABLE IF NOT EXISTS trades(id INTEGER PRIMARY KEY AUTOINCREMENT, ts TEXT, coin TEXT, side TEXT, qty REAL, entry REAL, sl REAL, tp1 REAL, exit REAL, exit_reason TEXT, pnl REAL, fees REAL, run_id TEXT)""")
        c.execute("""CREATE TABLE IF NOT EXISTS positions(coin TEXT PRIMARY KEY, side TEXT, qty REAL, entry REAL, sl REAL, tp1 REAL, trail_step REAL, open_ts TEXT, run_id TEXT)""")
        self.conn.commit()
...
def calc_indicators(df: pd.DataFrame):
    ...
    return {"rsi":float(rsi[-1]),"macd":float(macd[-1]),"macdsig":float(macdsig[-1]),"ema20":float(ema20[-1]),"ema50":float(ema50[-1]),"atr":float(atr[-1]),"price":float(close[-1])}
...
def detect_patterns(df: pd.DataFrame):
    ...
    for name,fn in pats.items():
        try:
            s=fn(o,h,l,c); 
            if not s.empty and int(s.iloc[-1])!=0: out.append(name)
        except: pass
    return out
...
def score_frame(ind, pat, funding=None, oi=None, tf="1h"):
    ...
    else: score-=0.3; reasons.append("MACD<Signal")
    if pat:
        reasons.append("Nến:"+", ".join(pat))
        if "Engulfing" in pat or "Hammer" in pat: score+=0.8
        if "ShootingStar" in pat: score-=0.8
    if tf=="1h":
...
class Bot5Ultra:
    def __init__(self):
        ...
        self.states={c: {"last_sig_1h":None,"last_bias_1h":0,"bars":COOLDOWN_BARS_1H} for c in self.coins}
        ...
        self.last_signal_hash = {}  # anti-spam per coin per hour
    ...
    def auto_tune_if_needed(self, force=False):
        ...
        # tune trên tập hợp: coins đang quét ∪ coins đang mở vị thế
...
/gc — housekeeping + *ép* Auto-Tune chạy ngay
...
"""
    ...
    def handle_commands(self, texts: List[str]):
        for t in texts:
            ...
            if low.startswith("/help"): self._cmd_help()
            ...
            elif low.startswith("/gc"): self.housekeeping_if_needed(force=True); self.tg.send("🧹 Đã dọn file log/DB (housekeeping).")
                ...
                if len(parts)>=2:
                    ...
                    if not pos: self.tg.send(f"ℹ️ Không có vị thế {coin}."); continue
                    price=binance_price(f"{coin}{QUOTE}")
                    if not price: self.tg.send("❌ Không lấy được giá thị trường."); continue
                    slip = (1 + SLIPPAGE_BPS/10000.0) if pos["side"]=="Short" else (1 - SLIPPAGE_BPS/10000.0)
                    self.db.close_pos(coin, price*slip, "ManualClose", FEE_RATE, self.tg.run_id)
                    self.tg.send(f"✅ Đóng {coin} {pos['side']} @ {price:.4f} (Manual)")
                else:
                    self.tg.send("ℹ️ Dùng: /close BTC")
            elif low.startswith("/flatten"):
                open_pos=self.db.get_open_positions()
                if not open_pos: self.tg.send("ℹ️ Không có vị thế nào đang mở."); continue
                closed=[]
                for coin,pos in list(open_pos.items()):
                    price=binance_price(f"{coin}{QUOTE}")
                    if not price: continue
                    slip = (1 + SLIPPAGE_BPS/10000.0) if pos["side"]=="Short" else (1 - SLIPPAGE_BPS/10000.0)
                    self.db.close_pos(coin, price*slip, "Flatten", FEE_RATE, self.tg.run_id)
                    closed.append(coin)
                self.tg.send("✅ Flatten xong: "+", ".join(closed) if closed else "❌ Flatten thất bại (không lấy được giá)")
    ...
    def one_coin_round(self, coin: str) -> Optional[float]:
        ...
        try:
            ...
            sc15,_=score_frame(ind15,pat15,tf="15m"); sc1h,_=score_frame(ind1h,pat1h,funding,oi,tf="1h"); sc4h,_=score_frame(ind4h,pat4h,tf="4h")
            s15=signal_from_score_static(sc15)
            tune_c=self.tune.get(coin, {"confirm":TUNE_CONFIRM_BASE,"alert":TUNE_ALERT_BASE})
            s1h=signal_from_score_dyn(sc1h, tune_c["confirm"], tune_c["alert"])
            ...
            # Điều kiện gửi confirm
            send_ok=True
            if ("Xác nhận" in s1h) and (st["last_sig_1h"]!=s1h):
                if st["bars"]<COOLDOWN_BARS_1H or note.startswith("🧯"): send_ok=False
            else: send_ok=False
            # Anti-spam 1h
            if send_ok:
            ...
            # Gửi & mở vị thế (nếu không ALERT_ONLY)
            if send_ok and self.confirmed_sent<MAX_CONFIRMED_PER_ROUND and not self.tg.paused and price:
                ...
                msg=[f"🤖 *BOT5 Ultra*", f"⏰ {now_str()}", f"📊 {coin}/USDT", f"🗳️ Vote: {vote}", f"1h: {s1h_pretty}"]
                if ALERT_ONLY:
                ...
                st["bars"]+=1; print(f"⏰ {now_str()} - {coin}: {s1h_pretty} | Vote: {vote}" + (f" | {note}" if note else ""))
            st["last_bias_1h"]=new_bias

            # REST-only: cập nhật SL/TP/trailing bằng giá hiện tại mỗi vòng
    ...
    async def main(self):
        self.tg.send(f"🚀 *BOT5 Ultra* (REST-only, Integrated v2) đang chạy — {now_str()}")
        ...
        while True:
            try:
                ...
                write_status({"time":now_str(),"state":"running","coins":self.coins,"paused":self.tg.paused,"pace_sec": PACE_SEC,
                              "alert_only": ALERT_ONLY, "paper_trade": PAPER_TRADE})
                self.confirmed_sent=0
                await asyncio.sleep(SLEEP_BETWEEN_ROUNDS)
                print("🔄 Bot 8 REST-only (Integrated v2) đang chạy... (xong 1 vòng)")
            except Exception as e:
